...
import 'package:hu_accomponist/integrations/imslp/Send_Strings_2Server.dart';
...
import 'package:hu_accomponist/features/search/Library_Edition_Picker.dart';
...
class SelectedSheet {
  ...
  /// The composer of the picked work.
  ///
  /// Carried explicitly because [MusicSheet] has no composer field — the
  /// name is known from the WorkSummary the user tapped, and used to be
  /// dropped at this boundary, which left the score screen unable to tell
  /// the feedback endpoint which piece was being played.
...
class _Music_Library_PageState extends State<Music_Library_Page> {
  ...
  void _openSheet(MusicSheet sheet, {required String composer}) async {
    ...
    try {
      ...
      Navigator.pop(
        context,
        SelectedSheet(sheet: sheet, pdfBytes: pdfBytes, composer: composer),
      );
    } catch (e) {
      setState(() {
